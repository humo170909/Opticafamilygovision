...
import { fechaLima, inicioSemanaLima, fechaLimaHaceN, inicioMesLima } from '../../utils/tiempo.js';

document.addEventListener('DOMContentLoaded', async () => {
  const _usuario = await checkAuth(['admin']);
  await initUI(_usuario);

  // Fecha en header — usar fechaLima() para evitar el desfase UTC
  const larga = formatDateLong(fechaLima());
...
async function cargarKPIs() {
  ...
  const { data: prods } = await supabase
    ...
    .eq('activo', true);
  const alertas = (prods || []).filter(p => p.stock_actual <= p.stock_minimo).length;
...
async function cargarUltimasVentas() {
  ...
  tbody.innerHTML = ventas.map(v => {
    ...
    const badge  = { efectivo:'bs', yape:'bi', plin:'bi', tarjeta:'bn', transferencia:'bw' }[v.metodo_pago?.toLowerCase()] || 'bn';
    return `
      <tr>
...
async function cargarTopProductos() {
  ...
  const { data, error } = await supabase
    ...
    .gte('created_at', inicioMesLima() + 'T00:00:00-05:00');
...
// ─── Rentabilidad del día (ventas − COGS − gastos operativos) ─────────────────
async function cargarRentabilidad() {
  const el  = (id) => document.getElementById(id);
  const hoy = fechaLima();

  // Paso 1: ventas y gastos del día (paralelo)
  const [{ data: ventas }, { data: gastos }] = await Promise.all([
    supabase.from(TABLAS.VENTAS)
      .select('id, total')
      .gte('created_at', hoy + 'T00:00:00-05:00')
      .lte('created_at', hoy + 'T23:59:59-05:00')
      .eq('estado', 'completada'),
    supabase.from(TABLAS.GASTOS)
      .select('monto')
      .eq('fecha', hoy),
  ]);

  const totalVentas = (ventas || []).reduce((s, v) => s + Number(v.total), 0);
  const gastosOp    = (gastos  || []).reduce((s, g) => s + Number(g.monto), 0);

  // Paso 2: COGS del día desde detalle_ventas.costo_unitario
  // Si la columna aún no existe (migración pendiente), queda en 0 sin romper nada.
  let cogsHoy = 0;
  ...
  if (ids.length) {
    ...
    if (!errCosto && detalles) {
      cogsHoy = detalles.reduce((s, d) => s + (Number(d.costo_unitario) || 0) * (d.cantidad || 0), 0);
    }
  }

  const totalCostos  = gastosOp + cogsHoy;
  const gananciaReal = totalVentas - totalCostos;

  if (el('rent-cogs-hoy'))    el('rent-cogs-hoy').textContent    = formatCurrency(cogsHoy);
  if (el('rent-costos-hoy'))  el('rent-costos-hoy').textContent  = formatCurrency(totalCostos);
  if (el('rent-ganancia-hoy')) {
    el('rent-ganancia-hoy').textContent = formatCurrency(gananciaReal);
    el('rent-ganancia-hoy').style.color = gananciaReal >= 0 ? 'var(--c-success)' : 'var(--c-danger)';
  }
}

// ─── Ventas por método de pago (últimos 30 días) ──────────────────────────────
async function cargarMetodosPago() {
  const contenedor = document.getElementById('dash-metodos-pago');
  if (!contenedor) return;

  const desdeStr = fechaLimaHaceN(30) + 'T00:00:00-05:00';

  const baseSelect = 'total, metodo_pago';
  ...
  if (error || !data?.length) {
    contenedor.innerHTML = '<p style="text-align:center;color:var(--c-ink-muted);font-size:.82rem;padding:20px 0;">Sin datos.</p>';
    return;
  }
  ...
  const metodosLabel = { efectivo: 'Efectivo', tarjeta: 'Tarjeta', transferencia: 'Transf.', yape: 'Yape', plin: 'Plin' };
  const colores      = { efectivo: '#4a90d9', tarjeta: '#2e9e6b', transferencia: '#d97a0f', yape: '#8b5cf6', plin: '#10b981' };

  contenedor.innerHTML = Object.entries(porPago)
    .sort((a, b) => b[1] - a[1])
    .map(([m, monto]) => {
      const pct   = Math.round((monto / totalPago) * 100);
      const label = metodosLabel[m] || m;
      const color = colores[m] || '#4a90d9';
      return `
        <div style="margin-bottom:11px;">
          <div style="display:flex;justify-content:space-between;align-items:center;font-size:.8rem;margin-bottom:4px;">
            <span style="font-weight:600;color:var(--c-ink);">${esc(label)}</span>
            <span style="color:var(--c-ink-muted);">${pct}% · ${formatCurrency(monto)}</span>
          </div>
...
async function cargarGraficoSemanal() {
  ...
  const fmtLima = (d) => new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Lima' }).format(d);
  for (let i = 6; i >= 0; i--) {
    const fechaLimaStr = fechaLimaHaceN(i);
    // getDay() del lunes UTC equivalente a esa fecha Lima
    const [y, m, d] = fechaLimaStr.split('-').map(Number);
    const dow = new Date(Date.UTC(y, m - 1, d)).getUTCDay();
    dias.push({ fecha: fechaLimaStr, label: i === 0 ? 'Hoy' : labels[dow] });
  }

  const { data: ventas } = await supabase
    .from('ventas')
    .select('created_at, total')
    .gte('created_at', dias[0].fecha + 'T00:00:00-05:00')
    .neq('estado', 'cancelada');

  const porDia = {};
  dias.forEach(d => { porDia[d.fecha] = 0; });
  (ventas || []).forEach(v => {
    // Convertir timestamp UTC a fecha Lima para agrupar correctamente
    const fecha = v.created_at ? fmtLima(new Date(v.created_at)) : null;
    if (fecha && porDia[fecha] !== undefined) porDia[fecha] += Number(v.total);
  });
