...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  el('tbody-usuarios')?.addEventListener('click', async (e) => {
    ...
    if (accion === 'toggle') {
      ...
      if (ok) {
        const { error } = await supabase.from('usuarios_perfil').update({ activo: nuevoEstado }).eq('id', id);
        if (error) { showToast('Error al actualizar el usuario: ' + error.message, 'error'); return; }
