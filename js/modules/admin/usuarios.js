...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  el('form-usuario')?.addEventListener('submit', async (e) => {
    ...
    if (id) {
      ...
      // Cambiar password si se escribió
      if (password && password.length >= 8) {
        const { error: errPwd } = await supabase.auth.admin.updateUserById(id, { password });
        if (errPwd) showToast('Usuario actualizado pero error al cambiar contraseña.', 'warning');
      }
      ...
      if (errAuth) { if (errEl) { errEl.textContent = errAuth.message; errEl.hidden = false; } if (btn) btn.disabled = false; return; }
      // Crear perfil
      await supabase.from('usuarios_perfil').upsert({ id: newUser.user.id, nombre, email, rol, activo });
