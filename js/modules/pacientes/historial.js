...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function renderTimeline(consultas) {
    ...
    panel.innerHTML = `<div class="timeline">${consultas.map((c, idx) => {
      ...
      const btnEliminar = _usuario?.rol === 'admin'
        ? `<button class="btn-icon --danger" data-action="eliminar-consulta" data-id="${c.id}" title="Eliminar consulta" style="margin-left:auto;flex-shrink:0;">
             <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/></svg>
           </button>`
        : '';
  ...
  async function cargarComprasPaciente(pacienteId) {
    ...
    const { data: ventas } = await supabase
      ...
      .select(`
        ...
        detalle_ventas(cantidad, productos(nombre, marca, modelo, codigo_barras)),
    ...
    cont.innerHTML = `<div class="timeline">${ventas.map((v, idx) => {
      ...
      const listaComp = (arr) => arr.map(c =>
        `<div class="compra-linea" style="flex-direction:column;align-items:flex-start;gap:2px;">
          <div style="display:flex;align-items:center;gap:5px;">
            ${svgCheck}
            <strong>${esc(c.tipo)}</strong>${c.descripcion ? `<span style="color:var(--c-ink-soft);font-weight:400;"> — ${esc(c.descripcion)}</span>` : ''}
          </div>
          <div style="font-size:.74rem;color:var(--c-ink-muted);padding-left:17px;">Cantidad: ${c.cantidad}</div>
        </div>`
      ).join('');
              ...
              ${prods.length ? `
                <div class="compra-section">
                  ...
                  ${prods.map(d => {
                    const p = d.productos || {};
                    const extras = [
                      p.marca         ? `Marca: ${esc(p.marca)}`         : null,
                      p.modelo        ? `Modelo: ${esc(p.modelo)}`        : null,
                      p.codigo_barras ? `Código: ${esc(p.codigo_barras)}` : null,
                    ].filter(Boolean).join(' · ');
                    return `<div class="compra-linea" style="flex-direction:column;align-items:flex-start;gap:2px;">
                      <div style="display:flex;align-items:center;gap:5px;">
                        ${svgCheck}
                        <strong>${esc(p.nombre || '—')}</strong>
                      </div>
                      ${extras ? `<div style="font-size:.74rem;color:var(--c-ink-muted);padding-left:17px;">${extras}</div>` : ''}
                      <div style="font-size:.74rem;color:var(--c-ink-muted);padding-left:17px;">Cantidad: ${d.cantidad}</div>
                    </div>`;
                  }).join('')}
                </div>` : ''}
  ...
  el('btn-nueva-consulta')?.addEventListener('click', () => {
    ...
    if (fechaInput) fechaInput.value = fechaLima();
  });
  ...
  el('panel-consultas')?.addEventListener('click', async (e) => {
    const btn = e.target.closest('[data-action="eliminar-consulta"]');
    if (!btn || btn.disabled) return;

    const consultaId = parseInt(btn.dataset.id);
    if (!consultaId) return;

    const ok = await confirmDialog(
      '¿Eliminar esta consulta?<br><small>Se eliminarán también la graduación y las recetas asociadas. Esta acción no se puede deshacer.</small>',
      { title: 'Eliminar consulta', confirmText: 'Sí, eliminar', confirmClass: 'btn-danger' }
    );
    if (!ok) return;
    ...
    if (pacienteSeleccionado) await cargarHistorial(pacienteSeleccionado.id);
  });
