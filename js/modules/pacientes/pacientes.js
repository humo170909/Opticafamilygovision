...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function abrirModal(paciente = null) {
    ...
    if (paciente) {
      ...
      if (hcFecha) hcFecha.value = fechaLima();
  ...
  async function cargarCompras(pacienteId) {
    ...
    const { data: ventas } = await supabase
      ...
      .select(`
        ...
        estado,
        created_at,
        detalle_ventas ( cantidad, precio_unitario, subtotal, productos(nombre, marca, modelo, codigo_barras) ),
    ...
    cont.innerHTML = ventas.map(v => {
      ...
      const tagsProd = (v.detalle_ventas || []).map(d => {
        ...
        const extras = [
          ...
          p.codigo_barras ? `Cód: ${esc(p.codigo_barras)}`    : null,
        ].filter(Boolean).join(' · ');
        return `<span class="compra-tag" style="display:flex;flex-direction:column;gap:1px;height:auto;padding:5px 9px;">
          <span style="font-weight:600;">${esc(p.nombre || '—')}</span>
          ${extras ? `<span style="font-size:.7rem;font-weight:400;opacity:.8;">${extras}</span>` : ''}
          <span style="font-size:.72rem;font-weight:400;opacity:.75;">×${d.cantidad} — S/${parseFloat(d.subtotal || 0).toFixed(2)}</span>
        </span>`;
      }).join('');
  ...
  async function guardarHC(pacienteId) {
    ...
    const fecha       = el('hc-fecha')?.value         || fechaLima();
