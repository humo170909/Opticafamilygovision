...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function cargarCompras(pacienteId) {
    ...
    const METODOS = { efectivo: 'Efectivo', tarjeta: 'Tarjeta', transferencia: 'Transferencia', yape: 'Yape / Plin' };
  ...
  async function guardarHC(pacienteId) {
    ...
    if (cId) {
      ...
      const { error: eUpd } = await supabase.from('consultas')
        ...
        .eq('id', cId);
      if (eUpd) {
        console.error('[HC] Error al actualizar consulta:', eUpd);
        showToast('Error al actualizar la consulta. Intenta de nuevo.', 'error');
        return;
      }
    } else {
      ...
      if (eIns) {
        ...
        showToast('Error al registrar la consulta. Intenta de nuevo.', 'error');
        return;
      }
      if (!ins) return;
      cId = ins.id;
    ...
    if (existGrad?.length) {
      ...
      if (eGrad) { console.error('[HC] Error al actualizar graduación:', eGrad); showToast('Error al actualizar la graduación.', 'error'); }
    } else {
      const { error: eGrad } = await supabase.from('graduaciones').insert({ ...grad, consulta_id: cId });
      if (eGrad) { console.error('[HC] Error al insertar graduación:', eGrad); showToast('Error al guardar la graduación.', 'error'); }
    }
  }
