...
async function iniciarWidgetAsistencia(usuario) {
  const { data: cfg } = await supabase.from('configuracion').select(
    'asistencia_hora_entrada,asistencia_hora_tardanza,asistencia_wsp_activo,asistencia_wsp_telefono,asistencia_wsp_apikey'
  ).limit(1).single();
  _attConfig = cfg || {};
...
function renderWidgetAsistencia(usuario) {
  ...
  if (!r || !r.hora_entrada) {
    ...
    if (infoEl) infoEl.innerHTML = `Entrada: <strong>${r.hora_entrada.slice(0,5)}</strong> &nbsp;${badge}`;
