...
const METODOS_GASTO_LABEL = {
  efectivo: 'Efectivo', yape: 'Yape', plin: 'Plin', transferencia: 'Transf.',
};

function setChipMetodo(metodo = 'efectivo') {
  const input = document.getElementById('g-metodo');
  if (input) input.value = metodo;
  document.querySelectorAll('#chip-metodo-gasto .pago-chip').forEach(chip => {
    chip.classList.toggle('active', chip.dataset.metodo === metodo);
  });
}
...
async function cargarGastos() {
  ...
  let query = supabase
    .from(TABLAS.GASTOS)
    .select('id, fecha, categoria, descripcion, monto, metodo_pago, observaciones, usuario_id, created_at')
  ...
  if (error) {
    tbody.innerHTML = `<tr class="empty-row"><td colspan="7">Error al cargar gastos.</td></tr>`;
...
function renderTabla(lista) {
  ...
  tbody.innerHTML = lista.map(g => {
    ...
    // La columna "Registrado por" usa admin-only — CSS la oculta al vendedor
    return `
      <tr>
        <td>${esc(formatDate(g.fecha))}</td>
        <td><span class="chip-categoria ${chipClass}">${esc(g.categoria)}</span></td>
        <td>${esc(g.descripcion)}</td>
        <td style="font-size:.82rem;">${esc(METODOS_GASTO_LABEL[g.metodo_pago] || g.metodo_pago || 'Efectivo')}</td>
        ...
        <td style="text-align:center;">
          <div class="td-acciones" style="justify-content:center;">
            <button class="btn-icon-sm" title="Ver detalle" onclick="verDetalle('${g.id}')">
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
            </button>
            ${puedeEditar ? `
            <button class="btn-icon-sm --warning" title="Editar" onclick="editarGasto('${g.id}')">
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
            </button>` : ''}
            ${puedeEliminar ? `
            <button class="btn-icon-sm --danger" title="Eliminar" onclick="eliminarGasto('${g.id}')">
...
window.verDetalle = function(id) {
  ...
  document.getElementById('detalle-subtitulo').textContent =
    `Registrado el ${g.created_at ? formatDate(new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Lima' }).format(new Date(g.created_at))) : '—'}`;
  ...
  document.getElementById('detalle-body').innerHTML = `
    <div class="detalle-grid">
      ...
      <div class="detalle-item">
        <div class="d-label">Método de pago</div>
        <div class="d-value">${esc(METODOS_GASTO_LABEL[g.metodo_pago] || g.metodo_pago || 'Efectivo')}</div>
      </div>
      <div class="detalle-item full-col">
...
window.editarGasto = function(id) {
  ...
  setChipMetodo(g.metodo_pago || 'efectivo');
...
async function guardarGasto(e) {
  ...
  const metodoPago    = document.getElementById('g-metodo')?.value || 'efectivo';
  ...
  const payload = { fecha, categoria, monto, descripcion, metodo_pago: metodoPago, observaciones: observaciones || null };
...
function inicializarEventos() {
  ...
  document.getElementById('chip-metodo-gasto')?.addEventListener('click', (e) => {
    const chip = e.target.closest('.pago-chip[data-metodo]');
    if (chip) setChipMetodo(chip.dataset.metodo);
  });
