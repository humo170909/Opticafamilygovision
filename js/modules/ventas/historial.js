        ...
        <tbody>
          ${items.map(i => `
            <tr style="border-bottom:1px solid var(--c-border-soft);">
              ...
              <td style="padding:7px 6px;text-align:right;color:var(--c-ink-soft);">${formatCurrency(i.precio_unitario)}</td>
    ...
    if (bodyEl) bodyEl.innerHTML = `
      ...
      <div class="detalle-info-grid" style="background:var(--c-bg);border-radius:var(--radius-sm);padding:12px 14px;">
        ...
        <div class="detalle-info-item">
          ...
          <div class="di-value" style="color:var(--c-danger);">${(venta.descuento * 100).toFixed(0)}%</div>
        </div>` : ''}
      </div>
