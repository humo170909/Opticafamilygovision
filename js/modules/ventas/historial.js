...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function cargarVentas() {
    ...
    // Intentar con venta_pagos (disponible tras ejecutar la migración SQL)
    let { data, error } = await aplicarFiltros(
      supabase.from('ventas').select(`
        id, created_at, total, descuento, metodo_pago, estado,
        paciente_id, created_by,
        pacientes(nombres, apellidos),
        detalle_ventas(id, cantidad, productos(nombre, codigo_barras)),
        venta_complementos(id, tipo, cantidad),
        venta_pagos(metodo, monto)
      `).order('created_at', { ascending: false }).limit(200)
    );

    // Fallback: si venta_pagos aún no existe (migración pendiente), reintenta sin ese join
    if (error) {
      ({ data, error } = await aplicarFiltros(
        supabase.from('ventas').select(`
          id, created_at, total, descuento, metodo_pago, estado,
          paciente_id, created_by,
          pacientes(nombres, apellidos),
  ...
  function renderTabla(ventas) {
    ...
    if (!ventas.length) {
      tbody.innerHTML = '<tr><td colspan="9" style="text-align:center;padding:32px;color:var(--c-ink-muted);">Sin ventas registradas.</td></tr>';
      return;
    }
    ...
    tbody.innerHTML = ventas.map(v => {
      ...
      const codigos = prods.map(d => d.productos?.codigo_barras).filter(Boolean);
      const codigosHtml = codigos.length
        ? codigos.map(c => `<div style="font-size:.75rem;font-family:var(--font-display);color:var(--c-ink-muted);">${esc(c)}</div>`).join('')
        : '<span style="color:var(--c-ink-faint);">—</span>';
      ...
      return `<tr>
        ...
        <td>${articulosHtml}</td>
        <td class="col-codigo">${codigosHtml}</td>
        <td><span style="font-family:var(--font-display);font-weight:700;">${formatCurrency(v.total)}</span></td>
        <td style="font-size:.82rem;">${esc(
          (v.venta_pagos || []).length > 1
            ? (v.venta_pagos || []).map(p => METODOS_LABEL[p.metodo] || p.metodo).join(' + ')
    ...
    const { data: venta } = await supabase
      ...
      .select(`
        *,
        ...
        detalle_ventas(*, productos(nombre, marca, modelo, codigo_barras)),
        venta_complementos(*)
      `)
      .eq('id', id)
      .single();
      ...
      <table style="width:100%;border-collapse:collapse;font-size:.82rem;margin-bottom:4px;">
        ...
        <tbody>
          ${items.map(i => {
            ...
            const p = i.productos || {};
            const detallesProd = [
              p.marca        ? `<span style="color:var(--c-ink-muted);">Marca:</span> ${esc(p.marca)}`        : null,
              p.modelo       ? `<span style="color:var(--c-ink-muted);">Modelo:</span> ${esc(p.modelo)}`      : null,
              p.codigo_barras ? `<span style="color:var(--c-ink-muted);">Código:</span> ${esc(p.codigo_barras)}` : null,
            ].filter(Boolean).join(' &nbsp;·&nbsp; ');
            return `<tr style="border-bottom:1px solid var(--c-border-soft);">
              <td style="padding:7px 10px;">
                <div style="font-weight:600;">${esc(p.nombre || '—')}</div>
                ${detallesProd ? `<div style="font-size:.72rem;color:var(--c-ink-soft);margin-top:2px;">${detallesProd}</div>` : ''}
              </td>
    ...
    if (bodyEl) bodyEl.innerHTML = `
      ...
      <div class="detalle-info-grid" style="background:var(--c-bg);border-radius:var(--radius-sm);padding:12px 14px;">
        ...
        <div class="detalle-info-item">
          ...
          <div class="di-value">${esc(
            (venta.venta_pagos || []).length > 1
              ? (venta.venta_pagos || []).map(p =>
                  `${METODOS_LABEL[p.metodo] || p.metodo}: S/ ${Number(p.monto).toFixed(2)}`
                ).join(' + ')
              : (METODOS_LABEL[venta.metodo_pago] || venta.metodo_pago || '—')
          )}</div>
        </div>
        ${(() => {
