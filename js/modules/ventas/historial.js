...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function cancelarVenta(id) {
    ...
    // RPC atómica: cancela la venta Y devuelve stock en una sola transacción.
    // Elimina el patrón SELECT+UPDATE que tenía race conditions con ventas concurrentes.
    const { error } = await supabase.rpc('restaurar_stock_cancelacion', { p_venta_id: id });
    if (error) { showToast('Error al cancelar la venta: ' + error.message, 'error'); return; }
