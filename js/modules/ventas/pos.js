...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function cargarProductos(busqueda = '', categoriaId = '') {
    let q = supabase.from('productos').select('id, nombre, marca, precio_venta, tipo_precio, precio_minimo, precio_maximo, stock_actual, imagen_url, categorias(nombre)').eq('activo', true).order('nombre');
  ...
  function agregarAlCarrito(id) {
    ...
    if (existing) {
      ...
      carrito.push({ id, nombre: producto.nombre, precio: producto.precio_venta, precioOriginal: producto.precio_venta, qty: 1, stock: producto.stock_actual, descItemTipo: 'ninguno', descItemValor: 0 });
    }
    renderCarrito();
  }
  ...
  function confirmarPrecioVariable() {
    ...
    carrito.push({ id: prod.id, nombre: prod.nombre, precio, precioOriginal: precio, qty: 1, stock: prod.stock_actual, esVariable: true, descItemTipo: 'ninguno', descItemValor: 0 });
    renderCarrito();
  ...
  el('btn-confirmar-venta')?.addEventListener('click', async () => {
    ...
    mostrarComprobante(venta, carrito, complementos, total, descMonto, metodoPago);
  });

  // ── Comprobante ───────────────────────────────────────────────────────────────
  function mostrarComprobante(venta, items, comps, total, descMonto, metodoPago) {
    ...
    // Limpiar carrito, complementos y estado de descuento
    carrito      = [];
    complementos = [];
    resetDescuentoUI();
    renderCarrito();
  ...
  el('btn-limpiar')?.addEventListener('click', async () => {
    ...
    if (ok) { carrito = []; complementos = []; resetDescuentoUI(); renderCarrito(); }
  });
