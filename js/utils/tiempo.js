...
/**
 * Retorna la fecha Lima de N días atrás como "YYYY-MM-DD".
 * Evita el error de usar new Date().toISOString() (UTC) para rangos de filtros.
 */
export function fechaLimaHaceN(n) {
  const hoy = fechaLima();
  const [y, m, d] = hoy.split('-').map(Number);
  return new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Lima' })
    .format(new Date(Date.UTC(y, m - 1, d - n)));
}

/** Retorna el primer día del mes Lima actual como "YYYY-MM-01". */
export function inicioMesLima() {
  return fechaLima().slice(0, 8) + '01';
}
